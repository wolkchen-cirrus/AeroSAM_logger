...
if __name__ == '__main__':
	...
	# usb_log.write_data_log(mavcon.boot_time, mavcon.press_hpa, mavcon.lat, mavcon.lon, mavcon.alt_m, mavcon.vz_ms,
			       # met_module.t_deg_c, met_module.rh_true, ss.hist, ss.mtof, ss.period, ss.checksum,
			       # ss.reject_glitch, ss.reject_ltof, ss.reject_ratio)
        mavcon.get_date_time()
        t2 = time()
        ...
        if (run_forever == False) & (loop_num > run_time):
            print("Loop Finished")
            break
